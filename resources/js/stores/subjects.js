...
export default {
    ...
    actions: {
        fetchSubjects({ state }) {
            return new Promise((resolve, reject) => {
                axios
                    ...
                    .catch(() =>
                        reject({ message: "Failed to fetch subjects" })
                    );
            });
        },
        ...
        saveSubject({ state, dispatch }, { id, formData }) {
            return new Promise((resolve, reject) => {
                axios
                    .post(`/admin/api/subjects/${id}`, formData)
                    .then(() => {
        ...
        deleteSubject({ dispatch }, subject_id) {
            return new Promise((resolve, reject) => {
                axios
                    .delete(`/admin/api/subjects/${subject_id}`)
                    .then(() => {
                        dispatch("fetchSubjects").catch(notify.error);
                        resolve();
                    })
                    .catch(() =>
                        reject({ message: "Failed to delete course." })
                    );
            });
        },
        ...
        publishSubject({ dispatch }, subject_id) {
            return new Promise((resolve, reject) => {
                axios
                    .post("/admin/api/public-subjects", { subject_id })
                    .then(() => {
                        dispatch("fetchSubjects").catch(notify.error);
                        resolve({ message: "Subject has been updated" });
                    })
                    .catch(() =>
                        reject({ message: "Unable to publish subject." })
                    );
            });
        },
        ...
        retractSubject({ dispatch }, subject_id) {
            return new Promise((resolve, reject) => {
                axios
                    .delete(`/admin/api/public-subjects/${subject_id}`)
                    .then(() => {
                        dispatch("fetchSubjects").catch(notify.error);
                        resolve({ message: "Subject has been retracted" });
                    })
                    .catch(() =>
                        reject({ message: "Unable to retract subject." })
                    );
            });
        },
        ...
        orderSubjects({ dispatch }, subject_ids) {
            return new Promise((resolve, reject) => {
                axios
                    .post("/admin/api/subjects-order", { subject_ids })
                    .then(() => {
                        dispatch("fetchSubjects").catch(notify.error);
                        resolve();
                    })
                    .catch(() =>
                        reject({ message: "Unable to set subject order" })
                    );
            });
        }
    }
