        ...
        <form action="" @submit.prevent="submitForm">
            <div class="p-8 bg-white shadow">
                ...
                <div class="flex justify-between">
                    ...
                    <div
                        class="my-4 ml-4 w-1/2"
                        :class="{
                            'border-b border-red-400': formErrors.nationality
                        }"
                    >
                        ...
                        <span
                            class="text-xs text-red-400"
                            v-show="formErrors.nationality"
                            >{{ formErrors.nationality }}</span
                        >
                        <select
                            id="nationality"
            ...
            <div class="p-8 bg-white my-12 shadow">
                ...
                <p class="text-gray-600 my-4">
                    Describe yourself in around 180 characters. Keep it brief
                    and interesting.
                </p>
                <div class="">
                    <div class="three-grid">
                        <div
                            v-for="(text, lang) in formData.bio"
                            class=""
                            :class="{
                                'border-b border-red-400': formErrors.bio
                            }"
                        >
                            <label class="form-label" :for="`bio_${lang}`">{{
                                lang
                            }}</label>
                            <span
                                class="text-xs text-red-400"
                                v-show="formErrors.bio"
                                >{{ formErrors.bio }}</span
                            >
                            <textarea
                                name="bio"
                    ...
                    <div v-if="missing_translations.length" class="mt-4">
                        ...
                        <button
                            class="sm-btn btn-navy uppercase mr-4"
                            v-for="language in missing_translations"
                            type="button"
                            @click="addBioTranslation(language)"
                        >
                            {{ language }}
                        </button>
                    </div>
            ...
            <div class="p-8 bg-white mt-12 shadow">
                ...
                <div
                    class="my-4"
                    :class="{
                        'border-b border-red-400': formErrors.teaching_since
                    }"
                >
                    <label class="form-label mb-4" for="teaching_since"
                        >When did you start teaching?</label
                    >
                    <span
                        class="text-xs text-red-400"
                        v-show="formErrors.teaching_since"
                        >{{ formErrors.teaching_since }}</span
                    >
                    <input
                        type="text"
                        class="input-text"
                        v-model="formData.teaching_since"
                        id="teaching_since"
                ...
                <div
                    class="my-8"
                    :class="{
                        'border-b border-red-400': formErrors.chinese_ability
                    }"
                >
                    <p class="form-label mb-4">Chinese ability</p>
                    <span
                        class="text-xs text-red-400"
                        v-show="formErrors.chinese_ability"
                        >{{ formErrors.chinese_ability }}</span
                    >
                    <div v-for="level in chinese_abilities" :key="level.value">
                        ...
                        />
                        <label :for="`zh_level_${level.value}`" class="pl-4">{{
                            level.description
                        }}</label>
                    </div>
                </div>
                <div
                    class="my-4 max-w-sm"
                    :class="{
                        'border-b border-red-400': formErrors.qualifications
                    }"
                >
                    ...
                    <span
                        class="text-xs text-red-400"
                        v-show="formErrors.qualifications"
                        >{{ formErrors.qualifications }}</span
                    >
                    <input
                        type="text"
                        name="qualifications"
...
export default {
    props: ["profile", "redirect-on-success"],
    ...
    methods: {
        setFormData() {
            this.formData = {
                ...this.profile,
                spoken_languages: this.profile.spoken_language_codes,
                nationality: this.profile.country_code
            };
        },

        submitForm() {
            this.$store
                .dispatch("me/saveProfile", this.formData)
                .then(() => this.$router.push(this.redirectOnSuccess))
                .catch(({ status, data }) => {
                    if (status === 422) {
        ...
        setFormErrors(errors) {
            Object.keys(errors).forEach(
                key => (this.formErrors[key] = errors[key][0])
            );
        },
