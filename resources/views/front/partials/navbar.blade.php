<div
    ...
    <div>
        <a href="{{ localUrl("/") }}" class="no-underline">
            @include('svg.logos.logo', ['classes' => 'h-6 hover:text-mustard'])
        </a>
    </div>
    <div
        ...
        <a class="type-a1 tracking-wide ml-4 mt-12 mb-4 md:mb-0 md:mt-0"
           href="{{ localUrl("/courses") }}">{{ trans('navbar.courses') }}</a>
        <a class="type-a1 tracking-wide ml-4 md:mb-0"
        ...
        <div class="hidden md:block group relative">
            ...
            <div class="nav-more hidden group-hover:block absolute center-x w-48">
                <div class="relative center-x top-8 up-pointer bg-hms-navy rounded px-4 pt-2 shadow-md">
        ...
        <a class="type-a1 tracking-wide ml-4" href="{{ transUrl(Request::path()) }}">中文</a>
    </div>
    <button class="md:hidden nav-trigger">
